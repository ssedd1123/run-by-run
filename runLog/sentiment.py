...
if __name__ == '__main__':
    ...
    parser.add_argument('-po', '--posOutput', required=True, help='Name of the human readible file to which positive runs are stored')
    parser.add_argument('-no', '--negOutput', required=True, help='Name of the human readible file to which negative runs are stored')
    ...
    with open(args.input) as f:
        result = json.load(f)
    if args.useAI:
        pos, neg = sentiment(result)
        intro = 'AI have selected %d runLog entries out of a total of %d for futher review.' % (len(pos), len(result))
        print(intro)
    else:
        pos = result
        neg = {}
